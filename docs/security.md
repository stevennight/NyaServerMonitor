...
报告来源地址作为节点的自动出口 IP 保存；节点本机报告中的网卡 IP 可能是内网地址，二者不会混用。国家/地区识别会在 controller 启动时以及有效展示 IP 变化时执行，并在数据库中记录已尝试的 IP；旧数据库中的自动地区值会在首次迁移时清空并按当前 IP 重建，结果只保存规范化的 `country_code`。人工 IP 或国家/地区覆盖不会被自动上报或识别结果替换，直到管理员清空覆盖值。默认 GeoIP 服务是 `https://ipwho.is/{ip}`，它会接收节点公网 IP；高隐私部署应通过 `NYASM_GEOIP_URL` 配置自建服务，或使用启动参数 `--geoip-url ""` 关闭该功能。
