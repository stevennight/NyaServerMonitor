...
- 节点通过 IPv4/IPv6 专用连接访问多个公网 IP 服务并上报出口 IP；成功结果缓存 5 分钟，失败结果 2 分钟后重试。出口 IP 和本机网卡 IP 分开展示；出口 IP、国家/地区支持人工覆盖，人工值不会被后续上报覆盖，清空后恢复自动值。
- controller 启动时会按节点当前上报的公网 IP 重新排队 GeoIP 识别；之后仅在 IP 变化时再次识别，结果以 `country_code`、`region`、`region_code`、`city` 和经纬度保存。GeoIP 地址可通过 `NYASM_GEOIP_URL` 或 `--geoip-url` 配置，默认使用 `https://ipwho.is/{ip}`。没有节点公网 IP 上报时不会使用报告连接来源地址兜底。
...
节点详情中的“出口 IP”来自节点按 IPv4/IPv6 强制连接第三方公网 IP 服务后的上报结果，IPv4 和 IPv6 分开保存；节点本机采集的网卡 IP 单独展示，可能是内网地址。若公网 IP 服务暂时不可用，节点保留最近一次成功结果并按退避时间重试。ipwho.is 返回的国家、Region、City 和经纬度会随当前出口 IP 保存；人工国家/地区覆盖同样只有在管理员清空后才会恢复自动识别。

未登录状态下成功返回的数据只有公开状态、公开节点聚合指标、公开实时摘要和初始化状态。公开节点会返回地区、分组、标签、平台、运行时长、累计流量和资源摘要，但不返回节点版本、IP、主机名、网卡名或检查目标。除 `GET /api/public/dashboard`、`GET /api/public/nodes/{id}/metrics`、`GET /api/public/telemetry/stream`、`GET /api/setup/status`、登录/初始化及子节点报告接口外，所有 API（包括私有指标、告警和通知渠道接口）都要求管理员会话；直接访问 `/admin` 未登录时只显示登录表单，不加载私有数据。

公开状态页仍会暴露资产数量、节点名称和可用性，节点名称不要包含域名、IP、机房、业务密钥或其他内部信息。地图可以按国家、Region 或 City 聚合；国家使用国家代码和世界底图标记，Region/City 使用 ipwho.is 返回的经纬度，缺失坐标的节点显示为待定位。地图支持缩放、点击聚合标记查看节点列表、展开全部节点并进入节点详情。Region/City 是 IP 地理库的估算位置，不是机器的精确物理位置；`region_code` 和城市文本也不承诺是跨服务统一的稳定 ID。若监控面板不应被互联网访问，应在反向代理、VPN 或防火墙层限制访问；公开页的 `noindex` 和短缓存不能替代访问控制。
...
健康检查在 node 启动后立即执行一次，之后随 `--interval` 周期执行；默认 15 秒，可配置范围为 5 秒到 24 小时。检查文件只在 node 启动时读取，修改 `/etc/nyasm/checks.json` 后需要重启 `nyasm-node`。

管理员节点详情会显示检查名称、目标、当前状态和耗时，并为 `http`、`tcp`、`ping` 保存和绘制历史检查图表。单选 Ping 时黄色虚线表示丢包率，HTTP/TCP 失败只显示红色失败点，多选时比较各检查的耗时曲线；详情打开期间每 15 秒静默刷新最新检查状态和历史样本。公开接口和公开详情不会返回检查目标或历史检查明细。

告警规则在管理后台的“告警管理”中维护。默认包含节点离线、服务失败、CPU/内存/磁盘阈值、延迟、丢包和 TLS 证书告警。通知渠道的目标和密钥使用 `NYASM_NOTIFICATION_KEY` 加密保存；未配置该变量时仍会记录告警事件，但不能创建通知渠道。添加渠道后可点击“发送测试”验证配置和连通性；测试消息不会生成告警事件。

GeoIP 识别由 controller 在启动时以及节点上报的有效公网 IP 变化时异步调用 ipwho.is，并把最后一次识别过的 IP 写入数据库；同一 IP 的普通心跳和报告不会重复调用。旧数据库中的自动地区值会在首次迁移时清空，再根据节点当前公网 IP 重新生成；可识别的旧手动地区值会转换为国家代码。地图的国家聚合使用规范化 `country_code`；Region 聚合使用 `country_code + region_code`（没有代码时使用规范化 Region 文本），City 聚合使用 `country_code + Region + city`，不会伪造不存在的城市 ID。GeoIP 服务会收到节点上报的公网 IP；如不希望使用外部服务，可启动时传入 `--geoip-url ""`，也可以配置自己的兼容接口。
