...
- controller 启动时会按节点当前上报的公网 IP 重新排队国家/地区识别；之后仅在 IP 变化时再次识别，结果以 `country_code` 保存并按中文映射显示。GeoIP 地址可通过 `NYASM_GEOIP_URL` 或 `--geoip-url` 配置，默认使用 `https://ipwho.is/{ip}`。没有节点公网 IP 上报时不会使用报告连接来源地址兜底。
...
节点详情中的“出口 IP”来自节点按 IPv4/IPv6 强制连接第三方公网 IP 服务后的上报结果，IPv4 和 IPv6 分开保存；节点本机采集的网卡 IP 单独展示，可能是内网地址。若公网 IP 服务暂时不可用，节点保留最近一次成功结果并按退避时间重试。人工国家/地区覆盖同样只有在管理员清空后才会恢复自动识别。
...
国家/地区识别由 controller 在启动时以及节点上报的有效公网 IP 变化时异步调用配置的 GeoIP 服务，并把最后一次识别过的 IP 写入数据库；同一 IP 的普通心跳和报告不会重复调用。旧数据库中的自动地区值会在首次迁移时清空，再根据节点当前公网 IP 重新生成；可识别的旧手动地区值会转换为国家代码。GeoIP 服务会收到节点上报的公网 IP；如不希望使用外部服务，可启动时传入 `--geoip-url ""`，也可以配置自己的兼容接口。
