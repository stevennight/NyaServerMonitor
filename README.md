...
- controller 启动时会按节点当前有效 IP 重新排队国家/地区识别；之后仅在 IP 变化时再次识别，结果以 `country_code` 保存并按中文映射显示。GeoIP 地址可通过 `NYASM_GEOIP_URL` 或 `--geoip-url` 配置，默认使用 `https://ipwho.is/{ip}`。
...
国家/地区识别由 controller 在启动时以及有效展示 IP 变化时异步调用配置的 GeoIP 服务，并把最后一次识别过的 IP 写入数据库；同一 IP 的普通心跳和报告不会重复调用。旧数据库中的自动地区值会在首次迁移时清空，再根据节点当前 IP 重新生成；可识别的旧手动地区值会转换为国家代码。GeoIP 服务会收到节点公网 IP；如不希望使用外部服务，可启动时传入 `--geoip-url ""`，也可以配置自己的兼容接口。
